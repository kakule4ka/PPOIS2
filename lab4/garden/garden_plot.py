...
from garden.exceptions import GardenLogicError, PlotCapacityError

class GardenPlot:
    def __init__(self):
        self.max_plants: int = 10
        self.soil: Soil = Soil()
    ...
    def plant_seed(self, species: str):
        if len(self.plants) >= self.max_plants:
            raise PlotCapacityError(f"Участок переполнен. Максимум {self.max_plants} растений.")
        self.tools[0].use(10)
