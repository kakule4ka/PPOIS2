...
from garden.exceptions import (
    ...
    ToolAlreadyPristineError,
    TankAlreadyFullError
)
...
class Tool:
    ...
    def repair(self):
        if self.durability == 100:
            raise ToolAlreadyPristineError(f"Инструмент '{self.name}' и так в идеальном состоянии.")
        self.durability = 100
...
class WateringSystem:
    ...
    def refill(self):
        if self.current_water == self.max_capacity:
            raise TankAlreadyFullError("Бак уже полностью заполнен.")
